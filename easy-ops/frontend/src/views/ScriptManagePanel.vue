    ...
    <div v-if="selectedFile" class="script-editor-section">
      ...
      <div class="node-status-bar" v-if="nodeConfigStatus.length > 0">
        ...
        <div class="node-chips">
          <div
            ...
            :title="`${node.nodeName}: ${getNodeSyncStatusLabel(node.syncStatus)}${node.contentHash ? ' (hash: ' + node.contentHash.substring(0, 8) + '...)' : ''}`"
          >
      ...
      <div v-else class="editor-container">
        ...
        <div v-else-if="!editContent && editContent !== ''" class="edit-placeholder">
          <a-empty description="点击「读取内容」加载配置文件" />
        </div>
      ...
      <div class="distribute-panel">
        <a-divider orientation="left">分发配置</a-divider>
      ...
      <a-form :label-col="{ span: 6 }" :wrapper-col="{ span: 16 }">
        <a-form-item label="文件路径" required>
          <a-input
            ...
            placeholder="绝对路径或相对路径，如 /app/scripts/start.sh"
          />
        </a-form-item>
...
const props = defineProps<{
  projectId: number
  projectNodes: Array<{ id: string; name: string; ip: string; status: number }>
}>()
...
async function loadFiles() {
  loading.value = true
  try {
    const res = await listScriptFiles(props.projectId)
    if (res.code === 200) {
...
async function loadFileSyncStatus(fileId: number) {
  try {
    const res = await getScriptSnapshot(props.projectId, fileId)
...
async function handleScan() {
  ...
  scanning.value = true
  try {
    const res = await scanScriptFiles(props.projectId, scanDir.value.trim())
    if (res.code === 200) {
      scriptFiles.value = res.data || []
...
async function loadContentAuto() {
  ...
  editError.value = null
  try {
    const res = await getScriptContentAuto(props.projectId, selectedFile.value.id)
    if (res.code === 200 && res.data) {
...
async function loadContentFromNode(nodeId: number) {
  ...
  editError.value = null
  try {
    const res = await getScriptContent(props.projectId, nodeId, selectedFile.value.id)
    if (res.code === 200) {
...
async function loadNodeConfigStatus() {
  ...
  try {
    const res = await getScriptSnapshot(props.projectId, selectedFile.value.id)
    if (res.code === 200 && res.data) {
...
async function handleDistribute() {
  ...
  try {
    const res = await distributeScript({
      projectId: props.projectId,
...
async function handleAddFile() {
  ...
  try {
    ...
    if (editingFile.value?.id) {
      res = await updateScriptFile(editingFile.value.id, model)
    } else {
      res = await createScriptFile(model)
    }
...
async function handleDelete(file: GlobalScriptFileModel) {
  if (!file.id) return
  try {
    const res = await deleteScriptFile(file.id, props.projectId)
    if (res.code === 200) {
...
function selectAllOnlineNodes() {
  distributeNodeIds.value = props.projectNodes
    .filter(n => n.status === 1)
    .map(n => Number(n.id))
}

function getNodeName(nodeId: number): string {
  const node = props.projectNodes.find(n => Number(n.id) === nodeId)
