...
**专家目录（固定）**： `~/.workbuddy/plugins/marketplaces/my-experts/plugins`。**禁止**将专家生成到其他目录。如果用户要求创建到其他路径，必须拒绝并说明："专家必须生成到专家目录才能被 WorkBuddy 检测到，其他目录生成后将无法使用。"然后使用专家目录继续执行。
...
**专家目录**：`~/.workbuddy/plugins/marketplaces/my-experts/plugins`
...
```bash
python3 scripts/init_expert.py <expert-name> --type agent|team --path ~/.workbuddy/plugins/marketplaces/my-experts/plugins
```
