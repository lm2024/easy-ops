...
```
~/.workbuddy/plugins/marketplaces/my-experts/plugins/%(name)s/
```

然后运行注册命令使其在 WorkBuddy 中可见：

```bash
python3 scripts/register_expert.py ~/.workbuddy/plugins/marketplaces/my-experts/plugins/%(name)s/
```
...
def main():
    ...
    # 硬检查：--path 必须是专家目录
    expected_path = Path.home() / '.workbuddy' / 'plugins' / 'marketplaces' / 'my-experts' / 'plugins'
    resolved_path = Path(output_path).expanduser().resolve()
    if resolved_path != expected_path:
        print(f"❌ Error: --path 必须为专家目录: {expected_path}")
        print(f"   当前传入: {resolved_path}")
        print(f"   非专家目录下创建的专家无法被 WorkBuddy 检测到。")
        sys.exit(1)
